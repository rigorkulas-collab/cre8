# Admin Portal Phase 4 Advanced Reports & Stylist Ranks Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Complete the Reports page by integrating detailed service usage metrics, stylist performance tables, and instant simulated Toast notifications for user export/print actions.

**Architecture:** Enrich the reports route component (`app/admin/reports/page.tsx`) to render two advanced cards: "Service Treatment Share" (with pure-CSS progress bars) and "Staff Performance Ledger" (ranked data grid). Add a zero-animation state-driven instant Toast alert overlay at the bottom-right of the viewport.
...
### Task 1: Integrate Service Share, Stylist Ranks, & Instant Toast in Reports Page

**Files:**
- Modify: `app/admin/reports/page.tsx`

- [ ] **Step 1: Locate and target the reports layout page**
  Open [page.tsx](file:///home/zachary/Desktop/cre8/app/admin/reports/page.tsx) to target the grid below the main bar chart to append our advanced analytical cards, and prepare local state handlers for the custom instant Toast banner.

- [ ] **Step 2: Modify `app/admin/reports/page.tsx`**
  Add mock dataset arrays for treatments and staff rankings, append the visual components below the chart card, and set up state-driven instant alert banners.

  *Complete revised file content for `app/admin/reports/page.tsx`:*
  ```tsx
  "use client";
  ...
  const MOCK_SERVICES_SHARE = [
    { name: "Signature Haircut & Style", bookings: 142, revenue: 35500, percent: 34 },
    { name: "Gel Nail Manicure Spa", bookings: 98, revenue: 24500, percent: 23 },
    { name: "Premium Keratin Therapy", bookings: 75, revenue: 41250, percent: 18 },
    { name: "Aromatherapy Full Massage", bookings: 62, revenue: 18600, percent: 15 },
    { name: "Organic Hydrating Facial", bookings: 42, revenue: 14700, percent: 10 },
  ];

  const MOCK_STAFF_PERFORMANCE = [
    { name: "Elena Rostova", bookings: 124, revenue: 86800, rating: 4.9, active: true },
    { name: "Aria Vance", bookings: 110, revenue: 77000, rating: 4.8, active: true },
    { name: "Marcus Chen", bookings: 96, revenue: 67200, rating: 4.7, active: true },
    { name: "Chloe Bennett", bookings: 88, revenue: 61600, rating: 4.9, active: true },
    { name: "Jordan Taylor", bookings: 72, revenue: 50400, rating: 4.6, active: true },
  ];
  ...
  export default function ReportsPage() {
    ...
    const triggerToast = (message: string) => {
      ...
      // Auto close after 2.5s simulated duration
      setTimeout(() => {
    ...
    return (
      <AdminLayout>
        ...
        <PageHeader 
          ...
          actions={
            <div className="flex items-center gap-2 select-none">
              <button 
                onClick={() => triggerToast("Print preview layout generated successfully.")}
              ...
              <button 
                onClick={() => triggerToast("System transaction ledger successfully exported to Excel (XLS).")}
            ...
            <div className="w-full h-64 border-b border-gray-200 relative flex items-end justify-between px-2 pt-6">
              ...
              {chartData.map((data, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Hover Value Tooltip Indicator (Pure-CSS display) */}
        ...
        {/* Phase 4 Advanced Analytical Columns Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full select-none">
          {/* Left Column: Service Treatment Distribution Shares */}
          <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6 flex flex-col gap-6 shadow-xs">
            <div className="flex flex-col text-left">
              <span className="text-sm font-black text-gray-900 tracking-tight">Service Treatment Shares</span>
              <span className="text-xs font-semibold text-gray-400 mt-0.5">
                Popularity percentages and accumulated revenue generation by treatment category
              </span>
            </div>

            <div className="flex flex-col gap-5">
              {MOCK_SERVICES_SHARE.map((service, index) => (
                <div key={index} className="flex flex-col gap-2">
                  {/* Service Metadata Row */}
                  <div className="flex justify-between items-center text-xs font-bold text-gray-900">
                    <span className="truncate max-w-xs">{service.name}</span>
                    <div className="flex items-center gap-3 shrink-0">
                      <span className="text-gray-400 font-semibold">{service.bookings} Bookings</span>
                      <span className="w-px h-3 bg-gray-200" />
                      <span className="text-emerald-600">₱{service.revenue.toLocaleString()}</span>
                    </div>
                  </div>

                  {/* Progressive Bar Overlay Container */}
                  <div className="w-full h-2.5 bg-gray-50 border border-gray-100 rounded-full overflow-hidden relative">
                    <div 
          ...
          {/* Right Column: Staff Stylist Performance Ledger */}
          <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6 flex flex-col gap-6 shadow-xs">
            <div className="flex flex-col text-left">
              <span className="text-sm font-black text-gray-900 tracking-tight">Staff Stylist Leaderboard</span>
              <span className="text-xs font-semibold text-gray-400 mt-0.5">
                Activity volume, direct billing totals, and automated rating indices
              </span>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left">
                <thead>
                  <tr className="border-b border-gray-100 pb-2 text-[9px] font-black uppercase text-gray-400 tracking-wider">
                    <th className="pb-3 pr-2">Stylist</th>
                    <th className="pb-3 px-2">Bookings</th>
                    <th className="pb-3 px-2">Sales Volume</th>
                    <th className="pb-3 pl-2 text-right">Rating</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 text-xs font-bold text-gray-700">
                  {MOCK_STAFF_PERFORMANCE.map((staff, idx) => (
                    <tr key={idx} className="hover:bg-gray-50/50">
                      {/* Name with Leader Medal */}
                      <td className="py-3 pr-2 flex items-center gap-2 text-gray-900">
                        {idx === 0 && <Award className="w-3.5 h-3.5 text-amber-500 shrink-0" />}
                        <span>{staff.name}</span>
                      </td>

                      {/* Bookings */}
                      <td className="py-3 px-2 text-gray-500 font-semibold">{staff.bookings} completed</td>

                      {/* Revenue */}
                      <td className="py-3 px-2 text-emerald-600">₱{staff.revenue.toLocaleString()}</td>
        ...
        {/* Global Instant Zero-Delay simulated Toast notification overlay */}
        {toastMessage && (
          <div className="fixed bottom-6 right-6 z-50 p-4 rounded-xl bg-gray-900 text-white shadow-2xl flex items-center gap-3 select-none border border-gray-800">
            <CheckCircle className="w-4 h-4 text-emerald-400 shrink-0" />
            <span className="text-xs font-bold">{toastMessage}</span>
            <button 
              onClick={() => setToastMessage(null)}
              className="text-gray-400 hover:text-white cursor-pointer ml-1.5 shrink-0"
            >
...
- Run local development compilation command to confirm zero TS or bundler errors in the updated reports view.
